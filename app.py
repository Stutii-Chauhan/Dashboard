    ...
    if st.checkbox("Show Advanced Visualizations"):
        ...
        if categorical_cols:
            ...
            for col in categorical_cols:
                ...
                if len(vc) > 1:
                    fig = px.pie(values=vc.values, names=vc.index, title=f"{col} (Top 5 Categories)")
        ...
        if numeric_cols:
            ...
            for col in numeric_cols:
                fig = px.box(df, y=col, points='all', title=f"Box Plot of {col}", labels={col: col}, hover_data=[col])
                fig.update_traces(marker=dict(size=6, opacity=0.6))
                fig.update_layout(yaxis_title=col, hoverlabel=dict(bgcolor="white", font_size=12))
                st.plotly_chart(fig, use_container_width=True)
        ...
        if datetime_cols and numeric_cols:
            ...
            if not valid_rows.empty and pd.api.types.is_datetime64_any_dtype(df[time_col]):
                fig = px.line(valid_rows.sort_values(by=time_col), x=time_col, y=metric_col, title=f"{metric_col} over time ({time_col})")
                st.plotly_chart(fig, use_container_width=True)
            else:
